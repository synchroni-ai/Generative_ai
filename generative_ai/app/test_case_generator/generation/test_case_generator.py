...
# #                 if llm_model == "mistral":
# #                     output = call_mistral(prompt, api_keys["together_ai"])
# #                 elif llm_model in ("gpt-4", "gpt-3.5-turbo", "gpt-4o"): # Use OpenAI
# #                     output = call_gpt4(prompt, api_keys["openai"], model=llm_model)
# #                 else:
# #                     raise ValueError(f"Unsupported LLM model: {llm_model}")
# #             except Exception as e:
# #                 print(f"❌ Error generating test case for {subtype} in document {doc_id}: {e}")
...
#     llm_model = inner_config.get("llm_model", "").lower()
#     llm_version = inner_config.get("llm_version") # Extract llm_version
#     subtypes = inner_config.get("subtypes", [])
#     temperature = inner_config.get("temperature", 0.7)
#     use_case = inner_config.get("use_case", [])
#     config_id = str(config.get("_id", uuid.uuid4()))
...
#                 if llm_model == "mistral":
#                     output = call_mistral(prompt, api_keys["together_ai"])
#                 elif llm_model == "openai": # Use OpenAI, check for llm_model == "openai"
#                     output = call_gpt4(prompt, api_keys["openai"], model=llm_version) # Use llm_version to specify the GPT model
#                 else:
#                     raise ValueError(f"Unsupported LLM model: {llm_model}")
#             except Exception as e:
#                 print(f"❌ Error generating test case for {subtype} in document {doc_id}: {e}")
