...
async def safe_generate(generate_func, *args, **kwargs):
    try:
        ...
        if "Credit limit exceeded" in str(e):
            raise HTTPException(
                status_code=402,
                detail={"message": "Credit limit exceeded. Please add credits or upgrade your plan.", "error_detail": str(e)}
            )
        else:
...
async def process_and_generate(
    ...
    try:
        ...
        for idx, chunk_text in enumerate(chunks, start=1):
            print(f"Processing chunk {idx}/{total_chunks} with model {model_name} for Test Cases")
        ...
        for idx, chunk_text in enumerate(chunks, start=1):
            print(f"Processing chunk {idx}/{total_chunks} with model {model_name} for User Stories")
        ...
        test_case_data = [{"Test Cases": tc} for tc in combined_test_cases.split('\n') if tc.strip()]
        user_story_data = [{"User Stories": us} for us in combined_user_stories.split('\n') if us.strip()]
 
        save_text_to_excel(test_case_data, excel_test_case_path, column_names=["Test Cases"])
        save_text_to_excel(user_story_data, excel_user_story_path, column_names=["User Stories"])
...
def delete_documents(document_ids: List[str]):
    try:
        ...
        return JSONResponse(content={"success": f"{len(document_ids)} documents deleted successfully."},
